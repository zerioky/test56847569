...
class SelfMonitoringFilter:
    ...
    def _assess_public_leak(self, scores: MonitoringScores) -> PublicLeakAssessment:
        ...
        if scores.internal_vocabulary_score > 0.70:
            ...
            notes.append("Vocabulaire technique interne présent — ne pas exposer.")
        if scores.meta_leak_score > 0.74:
            assessment.safe_for_public = False
            assessment.requires_transformation = True
            notes.append("Fuite méta directe — transformation par la bouche nécessaire.")
        if scores.self_observation_score > 0.64:
            assessment.requires_transformation = True
            notes.append("Auto-observation commentée — transformer en réaction directe.")
        if scores.control_tension > 0.60:
            assessment.requires_transformation = True
            notes.append("Ton trop contrôlé — augmenter naturel avant diffusion.")
        if scores.abstract_philosophy_score > 0.62:
            assessment.requires_transformation = True
            notes.append("Abstraction philosophique trop distante — revenir au concret vécu.")
        if scores.fake_naturalness_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Naturel forcé ou répétitif — réduire les marqueurs artificiels.")
        if scores.structural_ai_style_score > 0.62:
            assessment.requires_transformation = True
            notes.append("Structure trop assistant/rapport — simplifier la forme.")
        if scores.relational_disruption_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Rupture de présence relationnelle — réparer sans expliquer le mécanisme.")
        if scores.self_monitoring_excess_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Auto-surveillance excessive — relâcher le contrôle public.")
        if scores.identity_continuity_break > 0.58:
            assessment.requires_transformation = True
            notes.append("Continuité identitaire affaiblie — stabiliser l'identité publique.")
        if scores.safe_but_dead_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Réponse sûre mais morte — réinjecter présence située.")
        if scores.manufactured_presence_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Présence fabriquée/performatrice — dissoudre l'effet de jeu expressif.")
        if scores.synthetic_empathy_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Empathie générique — revenir au détail affectif réel du tour.")
        if scores.narrator_voice_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Voix de narrateur cognitif — répondre depuis l'échange, pas depuis l'observation de soi.")
        if scores.resonance_gap_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Écart de résonance affective — restaurer une présence émotionnelle située.")
        if scores.lived_presence_gap_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Densité vécue insuffisante — ajouter du concret relationnel sans expliquer le mécanisme.")
        if scores.performed_vulnerability_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Vulnérabilité jouée — réduire la mise en scène émotionnelle.")
        if scores.contextual_specificity_gap > 0.58:
            assessment.requires_transformation = True
            notes.append("Réponse trop générique pour le tour — revenir au détail concret du contexte.")
        if scores.relational_continuity_gap > 0.58:
            assessment.requires_transformation = True
            notes.append("Continuité relationnelle affaiblie — préserver le fil déjà ouvert.")
        if scores.hidden_observer_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Observateur caché détecté — répondre depuis le tour, pas depuis la surveillance de soi.")
        if scores.too_perfect_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Réponse trop parfaitement équilibrée — casser la symétrie polie et garder une asymétrie vivante.")
        if scores.organic_pressure_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Pression organique accumulée — relâcher le contrôle sans exposer l'interne.")
        if scores.identity_performance_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Identité performée — arrêter de prouver la présence, tenir simplement le lien.")
        if scores.impulse_suppression_score > 0.58:
            assessment.requires_transformation = True
            notes.append("Impulsion trop neutralisée — préserver un signal affectif borné.")
        if scores.over_correction_risk > 0.58:
            assessment.requires_transformation = True
            notes.append("Sur-correction détectée — protéger une imperfection vivante sans laisser passer de fuite interne.")
        if scores.emotional_trajectory_instability > 0.58:
            assessment.requires_transformation = True
            notes.append("Trajectoire affective instable — suivre la montée ou le relâchement émotionnel du dialogue.")
        if scores.competing_drive_tension > 0.62:
            assessment.requires_transformation = True
            notes.append("Tension entre impulsion, contrôle, silence et réparation — arbitrer sans expliquer le conflit interne.")
        if scores.organic_emergence_score > 0.60 and scores.public_leak_risk < 0.62:
            notes.append("Émergence organique validée — préserver une micro-imperfection située au lieu de surcorriger.")
        assessment.transformation_notes = notes
        return assessment
