...
class EmergentFrenchWeaver:
    ...
    def _repair(self, text: str) -> str:
        fixes = [
            ...
            (r"\s+t-il\b",   ""),
            # Corrections finales
            (r"\bje réponds une réponse\b", "je précise une réponse"),
            (r"\bla action\b",  "l'action"),
            (r"\bla image\b",   "l'image"),
            (r"\bla écoute\b",  "l'écoute"),
            (r"\bje cherche un doute\b",   "je précise une limite"),
            (r"\bje cherche une limite\b", "je précise une limite"),
            # Nettoyage espaces
            (r"\s+,", ","),
            (r"\s{2,}", " "),
        ]
        ...
        second_pass = [
            ...
            (r"\bje réponds une\b", "je précise une"),
            (r"\bje réponds un\b", "je précise un"),
        ]
    ...
    def _dynamic_atoms_from_payload(self, payload: Mapping[str, Any]) -> list[Atom]:
        ...
        asks_state = any(k in user_text for k in ("comment tu vas", "ça va", "ca va", "tu vas", "là maintenant", "la maintenant", "comment te sens"))
        if asks_state:
