...
class SubjectiveResponseIntegrator:
    ...
    def _atomically_recompose(self, user_input: str, payload: Mapping[str, Any], assessment: Mapping[str, Any]) -> str:
        # Surface construite avec fragments atomiques, pas avec phrases complètes par cas.
        lower = str(user_input or "").lower()
        fields = payload.get("semantic_field_weights", {}) if isinstance(payload.get("semantic_field_weights", {}), Mapping) else {}
        book_atoms = list(payload.get("book_public_atoms", []) or [])[:3]
        direct = self._asks_direct_truth(user_input)
        atoms: List[str] = []
        if direct:
            if any(x in lower for x in ("100%", "vivante", "consciente", "fini", "prête", "prete")):
                atoms.extend(["pas encore totalement", "mais", "plus stable"])
            elif any(x in lower for x in ("préécrit", "preecrit", "template")):
                atoms.extend(["pas en phrases fixes", "mais", "avec contraintes"])
            else:
                atoms.extend(["partiellement", "avec", "limite"])
        elif any(x in lower for x in ("vasy", "vas-y", "corrige", "ajoute")):
            atoms.extend(["je continue", "la correction", "dans", "le même noyau"])
        elif book_atoms:
            atoms.extend(["le livre", "change", "surtout"] + book_atoms[:2])
        else:
            top = [k for k, v in sorted(fields.items(), key=lambda kv: _clamp(kv[1]), reverse=True) if self._is_concrete_public_atom(k)][:3]
            atoms.extend(["je réponds", "plus directement"] + top)
        text = " ".join(a for a in atoms if a).strip()
    ...
    def _clean_surface(self, text: str) -> str:
        ...
        s = re.sub(r"\bje\s+(tiens|garde|relie)\s+(une|un|ce|cette)?\s*(continuité|continuite|résonance|resonance|appui|trace|lien|présence|presence|limite|état|etat)\b", "je reste plus précise", s, flags=re.I)
        s = re.sub(r"\bje\s+cherche\s+(un doute|une question)\b", "je précise ce qui manque", s, flags=re.I)
        s = re.sub(r"\s+([,.!?…])", r"\1", s)
