...
class ConversationWindow:
    ...
    def get_last_leia_response(self, offset: int = 0) -> str:
        ...
        idx = -(1 + offset)
        if abs(idx) <= len(turns):
            return turns[idx].get("leia", "")
        return ""
    ...
    def resolve_reference(self, text: str) -> str:
        ...
        Si le texte contient "ça", "cette question", "ce que tu as dit",
        etc., on retourne une version enrichie : le texte original suivi
        d'une annotation entre crochets pointant vers le dernier échange.

        On ne modifie pas le texte — on ajoute une annotation discrète
        uniquement si la confiance est suffisante.
        """
        if not self._turns:
            return text
        lower = _norm(text)
        ...
        last = self._turns[0]
        # Extrait un fragment représentatif de la réponse Leia
    ...
    def find_topic_in_history(self, topic_atoms: List[str], depth: int = 4) -> Optional[Dict[str, Any]]:
        ...
        Retourne le tour le plus pertinent ou None.
        """
    ...
    def signal_for_context(self) -> Dict[str, Any]:
        ...
        Contient :
          - recent_turns : liste des 4 derniers échanges
          - last_leia_said : dernière réponse textuelle
          - last_user_said : dernière entrée utilisateur
          - turn_count : nombre total de tours
          - available : True si au moins un tour existe
        """
        if not self._turns:
            return {
                "available": False,
